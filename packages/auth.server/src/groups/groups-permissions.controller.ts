...
export class ZGroupsPermissionsController {
  ...
  public async list(@Param() { groupId }: { groupId: string }): Promise<IZPermission[]> {
    ...
    const permissions = await this._dal.read<IZGroupPermission>(Collections.GroupsPermissions).join(Collections.Permissions, 'permissionId', '_id', 'permission').filter({ groupId }).run();
  ...
  public async update(@Param() { groupId, permissionId }: { groupId: string, permissionId: string }): Promise<IZPermission> {
    ...
    const assignment = new ZGroupPermissionBuilder().groupId(groupId).permissionId(permissionId).build();
    const [current] = await this._dal.read<IZGroupPermission>(Collections.GroupsPermissions).filter(assignment).run();
    ZHttpAssert.assert(!current, () => new ConflictException('That permission is already assigned to the group.'));
    await this._dal.create<IZGroupPermission>(Collections.GroupsPermissions, [assignment]).run();
    return permission;
  }
  ...
  public async remove(@Param() { groupId, permissionId }: { groupId: string, permissionId: string }): Promise<IZPermission> {
    const assignment = new ZGroupPermissionBuilder().groupId(groupId).permissionId(permissionId).build();
    const count = await this._dal.delete(Collections.GroupsPermissions).filter({ _id: assignment._id }).run();
