...
describe('ZDatabaseQuery', () => {
  ...
  it('sets the filter.', async () => {
    await assertOptions(
      filter,
      () => new ZDatabaseQuery(invoke).filter(filter),
      (q) => q.$filter
    );
  });
  ...
  it('sets the sort.', async () => {
    await assertOptions(
      sort,
      () => new ZDatabaseQuery(invoke).sorts(sort),
      (q) => q.$sort
    );
  });
  ...
  it('adds sorts.', async () => {
    ...
    await assertOptions(
      sort,
      () => new ZDatabaseQuery(invoke).sort(a.key, a.direction).sort(b.key, b.direction),
      (q) => q.$sort
    );
  });
  ...
  it('sets the join.', async () => {
    await assertOptions(
      join,
      () => new ZDatabaseQuery(invoke).joins(join),
      (q) => q.$join
    );
  });
  ...
  it('adds a join.', async () => {
    ...
    await assertOptions(
      join,
      () => new ZDatabaseQuery(invoke).join(a.from, a.local, a.foreign, a.as).join(b.from, b.local, b.foreign, b.as),
      (q) => q.$join
    );
  });
  ...
  it('sets the page.', async () => {
    await assertOptions(
      page,
      () => new ZDatabaseQuery(invoke).page(page),
      (q) => q.$page
    );
  });
  ...
  it('sets the page size.', async () => {
    await assertOptions(
      size,
      () => new ZDatabaseQuery(invoke).size(size),
      (q) => q.$size
    );
  });
