 ...
 */
export const ZHttpCodeSuccessNames = {
 ...
 */
export const ZHttpCodeSuccessDescriptions = {
  ...
  [ZHttpCodeSuccess.Accepted]: 'The request has been accepted for processing, but the processing has not been completed.',
  [ZHttpCodeSuccess.NonAuthoritativeInformation]: 'The server is a transforming proxy that received an OK from its origin,but is returning a modified version of the response.',
  [ZHttpCodeSuccess.NoContent]: 'The server successfully processed the request and is not returning any content.',
  [ZHttpCodeSuccess.ResetContent]: 'The server successfully processed the request, but is not returning any content.  The document view must be refreshed.',
  [ZHttpCodeSuccess.PartialContent]: 'he server is delivering only part of the resource due to a range header sent by the client.',
  [ZHttpCodeSuccess.MultiStatus]: 'The message body that follows is by default an XML message and can contain a number of separate response codes, depending on how many sub-requests were made.',
  [ZHttpCodeSuccess.AlreadyReported]: 'The members of a DAV binding have already been enumerated in a preceding part of the response, and are not being included again.',
  [ZHttpCodeSuccess.IMUsed]: 'The server has fulfilled a request for the resource, and the response is a representation of the result of one or more instance-manipulations applied to the current instance.'
};
