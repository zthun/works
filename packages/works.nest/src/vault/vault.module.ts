...
@Module({
  providers: [ZVaultService, ZCommonConfigService, { provide: ZVaultDatabase.Token, useValue: ZDatabaseMongo.connect(new ZDatabaseOptionsBuilder().database(ZVaultDatabase.Name).host(DATABASE_VAULT_HOST).port(DATABASE_VAULT_PORT).build()) }],
