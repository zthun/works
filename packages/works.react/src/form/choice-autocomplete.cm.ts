...
/**
 * Represents a component model for the choice autocomplete.
 */
export class ZChoiceAutocompleteComponentModel extends ZChoiceComponentModel {
  /**
   ...
   */
  private get _menu() {
    return document.body.querySelector<HTMLElement>('.ZChoice-autocomplete-menu');
  }
  ...
  public async open(): Promise<ZChoiceOptionComponentModel[]> {
    if (this._menu == null) {
      const query = '.ZChoice-autocomplete .MuiAutocomplete-input';
  ...
  public static find(container: Element): HTMLElement[] {
    return Array.from(container.querySelectorAll('.ZChoice-autocomplete'));
  }
}
